...
class GLTFParser {
  ...
  loadTexture(textureIndex) {
    ...
    return this.loadTextureImage(textureIndex, sourceIndex, loader).catch(
      function (error) {
        console.warn(
          `THREE.GLTFLoader: Error loading texture ${textureIndex}:`,
          error
        );
        return null;
      }
  ...
  loadTextureImage(textureIndex, sourceIndex, loader) {
    ...
    const promise = this.loadImageSource(sourceIndex, loader)
      .then(function (texture) {
        if (!texture) {
          console.warn(
            `THREE.GLTFLoader: Failed to load texture image for texture ${textureIndex}`
          );
          return null;
        }
      ...
      .catch(function (error) {
        console.warn(
          `THREE.GLTFLoader: Error in loadTextureImage for texture ${textureIndex}:`,
          error
        );
        return null;
      });
  ...
  loadImageSource(sourceIndex, loader) {
    ...
    if (sourceDef.bufferView !== undefined) {
      ...
      sourceURI = parser
        ...
        .catch(function (error) {
          console.warn(
            `THREE.GLTFLoader: Failed to get bufferView for image ${sourceIndex}:`,
            error
          );
          return null;
        });
      ...
      .then(function (sourceURI) {
        ...
        return new Promise(function (resolve, reject) {
          ...
          loader.load(
            ...
            function (error) {
              console.warn(
                "THREE.GLTFLoader: Failed to load texture image:",
                sourceURI,
                error
              );
              resolve(null); // 返回 null 而不是 reject
            }
          );
        });
      })
      ...
      .catch(function (error) {
        ...
        if (isObjectURL === true && objectURL) {
          try {
            ...
            console.warn(
              "THREE.GLTFLoader: Error revoking object URL on error:",
              e
            );
          }
        }
  ...
  assignTexture(materialParams, mapName, mapDef, colorSpace) {
    ...
    return this.getDependency("texture", mapDef.index)
      .then(function (texture) {
        if (!texture) {
          console.warn(
            `THREE.GLTFLoader: Texture for ${mapName} failed to load, using default material`
          );
          return null;
        }
      ...
      .catch(function (error) {
        console.warn(
          `THREE.GLTFLoader: Failed to assign texture for ${mapName}:`,
          error
        );
        // 返回 null 而不是抛出错误，让材质继续加载
        return null;
      });
  }
  ...
  loadScene(sceneIndex) {
    ...
    for (let i = 0, il = nodeIds.length; i < il; i++) {
      const node = pending.push(parser.getDependency("node", nodeIds[i]));
    }
